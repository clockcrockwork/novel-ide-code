...
> **Ground truth:** The retrieved guide(s) from `modern-web-guidance` (the preferred local standard); the user's stated browser-support policy if any.
> **Entry gate:** Search and retrieve the relevant guide before writing UI / CSS / client-side JS; do not implement ad-hoc patterns first.
> **Required artifacts:** Note the guide `id`(s) consulted; record the confirmed guidance category in the PR body (per CLAUDE.md「Web 実装ガイダンス」).
> **Verification gate:** Change-type gates → [docs/ai/rules/verification-gates.md](../../../docs/ai/rules/verification-gates.md) (UI/CSS/JS changes run `npm run lint` / `npm run test` / `npm run build`).
> **Anti-skip:** See [docs/ai/README.md](../../../docs/ai/README.md)「anti-skip rule」; do not skip the search because a pattern "looks obvious" — web APIs evolve and training weights go stale.
> **Cost note:** Skipping leads to obsolete patterns, unnecessary dependencies, and accessibility/compat regressions.
