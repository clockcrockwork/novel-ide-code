...
OpenAI Codex（および同等の AI エージェント）向けの **novel-ide 入口**です。

## Start here

作業開始時は [`docs/ai/README.md`](docs/ai/README.md) を読み、タスク種別に応じてそこから正本の rule / workflow / checklist へ進んでください。

**詳細ルール・作業手順・チェックリストの本文をこのファイルへ複製しません。** 変更時は正本だけを更新します。
...
定型作業は [`docs/agent-workflows/`](docs/agent-workflows/) を正とします。特に実装前調査・risk-modeling・review・evidence-check 等を入口ファイル側へ再掲しません。
