# GitHub Copilot 向けプロジェクトガイド

GitHub Copilot 向けの **novel-ide 入口**です。

## Start here

作業開始時は [`docs/ai/README.md`](../docs/ai/README.md) を読み、タスク種別に応じてそこから正本の rule / workflow / checklist へ進んでください。

Copilot 固有の起動・実行設定だけをこの入口で扱い、**詳細ルール・作業手順・チェックリストの本文は repository の正本へ置きます。**
...
- タスク開始時の role / delegation 計画は [`docs/agent-workflows/subagent-roles.md`](../docs/agent-workflows/subagent-roles.md) を正とし、実装前に確認する。
...
- レビュー・作業説明は日本語を基本とする。詳細は [`docs/ai/rules/communication.md`](../docs/ai/rules/communication.md) に従う。
